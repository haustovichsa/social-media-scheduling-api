 ...
 * no parent, or a parent `parentOf` can't resolve (split across a page boundary,
 * or missing from the batch). Cycle-safe: a chain that loops back on itself stops
 * rather than spinning forever.
 *
 * `parentOf` lets the same walk serve different backings — an in-batch `Map` for
 * the read path, the live store for the mock's reply path.
 */
...
/**
 * The ancestor of `node` at `targetDepth`, walking up from `node`. Hops =
 * `depth(node) - targetDepth`, computed once so the chain is walked a single
 * time. If the chain ends early (missing or looping parent) the highest reached
 * node is returned.
 */
...
/**
 * Enforce a platform's max reply-nesting depth on a batch of fetched comments.
 * Platforms cap threading at different depths — Facebook, for instance, collapses
 * a reply-to-a-reply onto the top-level comment — so an adapter runs its page
 * through here: a comment deeper than `maxDepth` is re-parented onto its deepest
 * still-allowed ancestor rather than dropped, so no comment is lost.
 *
 ...
 * `externalParentCommentId`, measuring every depth against the original links.
