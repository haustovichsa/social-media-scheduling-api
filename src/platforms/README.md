...
2. **Write the adapter.** Create an `@Injectable()` class implementing
   ...
   the account's token and normalises a failure-to-resolve into a
   `TokenExpiredError` for you. Never read a token directly (RK-6). **Throw only
   `PlatformError` subclasses** — a raw vendor error escaping the adapter is a
   bug.
